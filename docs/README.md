...
- [`plans/user-themes.md`](plans/user-themes.md) — **done.** Users drop a
  `*.toml` file into a `themes/` config dir and it shows up in the theme picker
  like a built-in. A serde DTO + base-overlay merge + directory scan
  (`crates/nyx/src/theme_load.rs`); the only `nyx-ui` change is
  `Theme.name: &'static str → String`, so Flint stays serde-free. User guide:
  [`user/custom-themes.md`](user/custom-themes.md).
